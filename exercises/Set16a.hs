...
sumIsLength input output = foldr (\(_, i) d -> i + d) 0 output === length input
...
outputInInput input output = forAll (elements output) $ \(a, c) -> count a === c
  where
    count a = length $ filter (== a) input
...
genList = do
  l <- choose (3, 5)
  fmap sort $ vectorOf l $ elements [0 .. 10]
...
instance Arbitrary Arg where
  arbitrary = elements $ map Number [0 .. 10] ++ map Variable ['a', 'b', 'c', 'x', 'y', 'z']

instance Arbitrary Expression where
  arbitrary = elements $ [Plus, Minus] <*> args <*> args
    where
      args = map Number [0 .. 10] ++ map Variable ['a', 'b', 'c', 'x', 'y', 'z']
