...
prettyPrint n coords = prettyPrint' n (sortBy (\(a, b) (c, d) -> if a < c || (a == c && b < d) then LT else GT) coords) (1, 1)
  where
    prettyPrint' n [] (y, x)
      | x > n = '\n' : prettyPrint' n [] (y + 1, 1)
      | y > n = []
      | otherwise = '.' : prettyPrint' n [] (y, x + 1)
    prettyPrint' n qs (y, x)
      | (y, x) == head qs = 'Q' : prettyPrint' n (tail qs) (y, x + 1)
      | x > n = '\n' : prettyPrint' n qs (y + 1, 1)
      | y > n = []
      | otherwise = '.' : prettyPrint' n qs (y, x + 1)
...
-- by considering Pythagoras, these two tests turn out the exact same
-- the triangle has a right angle and the two others are pi/4, which is how we arrive at this
...
danger cand = any (\q -> sameRow cand q || sameCol cand q || sameDiag cand q)
...
prettyPrint2 n qs = prettyPrint2' n qs (1, 1)
  where
    prettyPrint2' n qs (y, x)
      | x > n = '\n' : prettyPrint2' n qs (y + 1, 1)
      | y > n = []
      | (y, x) `elem` qs = 'Q' : prettyPrint2' n qs (y, x + 1)
      | (y, x) `danger` qs = '#' : prettyPrint2' n qs (y, x + 1)
      | otherwise = '.' : prettyPrint2' n qs (y, x + 1)
...
step n qs = case fixFirst n qs of
  ...
  Just (x : xs) -> nextRow (head qs) : x : xs
