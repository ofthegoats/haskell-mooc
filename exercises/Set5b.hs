...
treeMax Empty          = 0
treeMax (Node a t1 t2) = max (max (treeMax t1) (treeMax t2)) a
...
allValues cond (Node a t1 t2) = cond a && (allValues cond t1 && allValues cond t2)
...
cull _ Empty = Empty
cull val (Node a t1 t2) = if val == a then Empty
...
isOrdered (Node a t1 t2) = allValues (<=a) t1 && isOrdered t1
                        && allValues (>=a) t2 && isOrdered t2
...
walk (StepL:steps) (Node _ lt _) = walk steps lt
walk (StepR:steps) (Node _ _ rt) = walk steps rt
...
set _ _ Empty                        = Empty
set [] val (Node a lt rt)            = Node val lt rt
set (StepL:steps) val (Node a lt rt) = Node a (set steps val lt) rt
set (StepR:steps) val (Node a lt rt) = Node a lt (set steps val rt)
...
search val (Node a lt rt) = if val == a then Just []
                            else case search val lt of
                                 Just steps -> Just $ StepL : steps
                                 Nothing -> case search val rt of
