...
data State = Start | HasEggs | HasFlour | HasSugar | HasFlourSugar | IsMixed | Error | Finished
  deriving (Eq, Show)
...
step HasSugar AddFlour = HasFlourSugar
step HasFlour AddSugar = HasFlourSugar
step HasFlourSugar Mix = IsMixed
...
instance (Ord a) => Semigroup (Set a) where
  Set as <> Set [] = Set as
  Set [] <> Set bs = Set bs
  Set (a : as) <> Set bs = Set as <> add a (Set bs)
...
passwordAllowed password (And crit1 crit2) = passwordAllowed password crit1 && passwordAllowed password crit2
passwordAllowed password (Or crit1 crit2) = passwordAllowed password crit1 || passwordAllowed password crit2
passwordAllowed password (MinimumLength n) = length password >= n
passwordAllowed [] (ContainsSome str) = False
passwordAllowed (p : ps) (ContainsSome str) = foldr (\s d -> s == p || d) False str || passwordAllowed ps (ContainsSome str)
passwordAllowed [] (DoesNotContain str) = True
passwordAllowed (p : ps) (DoesNotContain str) = foldr (\s d -> s /= p && d) True str && passwordAllowed ps (DoesNotContain str)
...
data Arithmetic
  = Number Integer
  | BinOp String Arithmetic Arithmetic
  deriving (Show)
...
evaluate (Number x) = x
evaluate (BinOp "+" ar1 ar2) = evaluate ar1 + evaluate ar2
evaluate (BinOp "*" ar1 ar2) = evaluate ar1 * evaluate ar2

render :: Arithmetic -> String
render (Number x) = show x
render (BinOp op ar1 ar2) = "(" ++ render ar1 ++ op ++ render ar2 ++ ")"
