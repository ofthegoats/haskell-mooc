...
buyOneMore (MkShoppingEntry item p amount) = MkShoppingEntry item p (succ amount)
...
toNat z
  | z == 0 = Just Zero
  | z < 0 = Nothing
  | otherwise = case toNat (z-1) of Just w -> Just . PlusOne $ w
...
fromBin :: Bin -> Int
fromBin = fromBin' 1
  where fromBin' m End   = 0
        fromBin' m (O b) = fromBin' (m*2) b
        fromBin' m (I b) = m + fromBin' (m*2) b

-- I have entirely by myself discovered a fairly efficient algorithm for writing binary
-- numbers in reverse!!! (note: this does not necessarily mean I found it first, I probably didn't)
toBin :: Int -> Bin
toBin 0 = O End
toBin 1 = I End
toBin target
  | even target = O . toBin $ target `div` 2
  | odd target  = I . toBin $ target `div` 2
