...
split str
  | length names > 1 = Just (head names, last names)
  | otherwise = Nothing
  where
    names = words str
...
checkNumber (for, sur)
  | all isAlpha for && all isAlpha sur = Just (for, sur)
  | otherwise = Nothing
...
winner scores p1 p2 = lookup p1 scores ?> \s1 -> lookup p2 scores ?> \s2 -> if s1 >= s2 then Just p1 else Just p2
...
selectSum vs is = selectSum' vs is 0
  where
    selectSum' vs [] s = Just s
    selectSum' vs (i : is) s = safeIndex vs i >>= \x -> selectSum' vs is $ s + x
...
countAndLog predicate xs = Logger (fmap show valid) (length valid)
  where
    valid = filter predicate xs
...
paren '(' = get >>= \x -> if x == -1 then put x else put $ x + 1
paren ')' = get >>= \x -> if x == -1 then put x else put $ x - 1
paren _ = get >>= put
...
count x = get >>= \xs -> put $ f xs
  where
    f [] = [(x, 1)]
    f ((y, z) : xs) = if x == y then (y, z + 1) : xs else (y, z) : f xs
...
occurrences = foldr ((>>) . count) (get >>= \x -> return $ length x)
