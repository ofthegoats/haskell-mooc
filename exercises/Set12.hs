...
instance Functor List where
  fmap f Empty = Empty
  fmap f (LNode a ls) = LNode (f a) (fmap f ls)
...
count want = foldr (\x -> if x == want then (+ 1) else (+ 0)) 0
...
inBoth xs ys = foldr (\x -> if x `elem` ys then (x :) else ([] ++)) [] xs
...
instance Foldable List where
  ...
  foldr f def (LNode a rest) = f a (foldr f def rest)
...
instance Foldable TwoList where
  ...
  foldr f def (TwoNode a b rest) = f a (f b (foldr f def rest))
...
-- The examples above suggest sumTree is just an inorder traversal using a Monoid
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
