...
statements xs ys = concat $ formPhrases <$> xs <*> ys
  where
    formPhrases x y = [x ++ " is " ++ y, x ++ " is not " ++ y]
...
calculator op n
  | op == "negate" = Just negate <*> readMaybe n :: Maybe Int
...
validateDiv n d = check (d /= 0) "Division by zero!" (n `div` d)
...
validateAddress streetName streetNumber postCode = Address <$> checkStreetName streetName <*> checkStreetNumber streetNumber <*> checkPostCode postCode
  where
    checkStreetName s = check (length s <= 20) "Invalid street name" s
    checkStreetNumber s = check (all isDigit s) "Invalid street number" s
    checkPostCode s = check (length s == 5 && all isDigit s) "Invalid postcode" s
...
twoPersons name1 age1 employed1 name2 age2 employed2 = (\x y -> [x, y]) <$> person1 <*> person2
  where
...
boolOrInt str = parseBool (readMaybe str :: Maybe Bool) <|> parseInt (readMaybe str :: Maybe Int)
  where
    parseBool Nothing = invalid "Not a Bool"
    parseBool (Just b) = pure $ Left b
    parseInt Nothing = invalid "Not an Int"
    parseInt (Just i) = pure $ Right i
...
normalizePhone xs = check (length xs' <= 10) "Too long" xs *> traverse (\x -> check (isDigit x) ("Invalid character: " ++ [x]) x) xs'
  where
    xs' = strip xs
    strip [] = []
    strip (' ' : xs) = strip xs
...
parseExpression str
  | (length . words) str /= 3 = invalid $ "Invalid expression: " ++ str
  | otherwise = let [a, op, b] = words str in buildExpr <$> parseArg a <*> parseOp op <*> parseArg b
  where
    buildExpr x y z
      | y == "+" = Plus x z
      | y == "-" = Minus x z
    parseArg x = case readMaybe x :: Maybe Int of
      Just x' -> pure $ Number x'
      Nothing ->
        invalid ("Invalid number: " ++ x)
          <|> if length x == 1 && isAlpha (head x)
            then pure $ Variable (head x)
            else invalid $ "Invalid variable: " ++ x
    parseOp o = check (o == "+" || o == "-") ("Unknown operator: " ++ o) o
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap fun (Both x) = Both $ fmap fun <$> x
...
instance (Applicative f, Applicative g) => Applicative (Both f g) where
  ...
  liftA2 f (Both x) (Both y) = Both $ liftA2 f <$> x <*> y
